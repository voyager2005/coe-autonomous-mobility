...
def cleanup_checkpoints(ckpt_dir):
    """
    Automatically selects the best checkpoint (lowest test loss) and removes all others.
    Called after training completes.
    """
    # Grab all the epoch files the script dumped
    files = glob.glob(os.path.join(ckpt_dir, 'embedder_model_epoch*.tar'))
    ...
    # Loop through and parse the floats out of filenames
    for file_path in files:
        filename = os.path.basename(file_path)
        try:
            # Format: embedder_model_epochX_TrainAcc_TrainLoss_TestAcc_TestLoss.tar
            parts = filename.replace('.tar', '').split('_')
            test_loss = float(parts[-1]) # Grab the last number (TestLoss)
    ...
    if best_file:
        print(f"\n{'='*60}")
        print(f"Champion model identified: {os.path.basename(best_file)}")
        print(f"Test Loss: {lowest_loss:.4f}")
        print(f"{'='*60}")
        
        # Rename it
        best_new_path = os.path.join(ckpt_dir, 'embedder_best.tar')
        if os.path.exists(best_new_path):
            os.remove(best_new_path) 
        os.rename(best_file, best_new_path)
        print("✓ Renamed to: embedder_best.tar")
        
        # Purge the remaining files
        removed_count = 0
        for file_path in files:
            if file_path != best_file and os.path.exists(file_path):
                os.remove(file_path)
                removed_count += 1
        
        print(f"✓ Removed {removed_count} older checkpoint(s)")
        print(f"\nCleanup complete! Ready to train the main transformer.\n")
    else:
        print("Failed to parse the filenames. Check your directory.")
...
if __name__ == "__main__":
    ...
    # Automatically clean up checkpoints and keep only the best one
    cleanup_checkpoints(args.check_dir)
